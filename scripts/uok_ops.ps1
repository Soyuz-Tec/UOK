...
function Invoke-UokBackupDb {
    ...
    Invoke-UokStep "Create PostgreSQL custom dump" {
        Invoke-Native "podman" @("exec", $container, "pg_dump", "-U", "uok", "-d", "uok", "--format=custom", "--no-owner", "--no-acl", "--file", "/tmp/$fileName")
    }
...
function Invoke-UokRestoreDb {
    ...
    try {
        ...
        Invoke-UokStep "Restore PostgreSQL dump" {
            Invoke-Native "podman" @("exec", $container, "pg_restore", "-U", "uok", "-d", "uok", "--clean", "--if-exists", "--no-owner", "--no-acl", "/tmp/$fileName")
        }
    } finally {
        Invoke-Native "podman" @("exec", $container, "rm", "-f", "/tmp/$fileName")
