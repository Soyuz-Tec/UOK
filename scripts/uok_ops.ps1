...
function Invoke-UokVerify {
    ...
    try {
        ...
        Invoke-UokStep "Frontend bundle budget" { Invoke-Native "npm" @("run", "check:bundle-budget") }
        Invoke-UokStep "Frontend accessibility proof" { Invoke-Native "npm" @("run", "test:accessibility") }
    } finally {
        Pop-Location
    }
    Invoke-UokUiProof
