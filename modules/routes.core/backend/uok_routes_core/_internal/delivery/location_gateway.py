...
def resolve_path_locations(
    db: Session,
    actor: Actor,
    location_definition_ids: Iterable[str],
) -> tuple[LocationReferenceResolution, ...]:
    return resolve_location_references(db, actor, location_definition_ids)


def require_active_path_locations(
    db: Session,
    actor: Actor,
    location_definition_ids: Iterable[str],
) -> tuple[LocationReferenceResolution, ...]:
    resolutions = resolve_path_locations(db, actor, location_definition_ids)
    ...
    if invalid:
        ...
        raise ValueError(f"route path Locations must be active and visible ({states})")
    return resolutions
...
__all__ = [
    ...
    "require_active_path_locations",
    "resolve_path_locations",
]
