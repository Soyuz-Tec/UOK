...
def cmd_merge_duplicate_contact(db: Session, actor: Actor, payload: dict[str, Any], command_id: str) -> dict[str, Any]:
    ...
    moved_notes = move_notes(db, actor, primary, duplicate)
    moved_groups = move_group_memberships(db, actor, primary, duplicate)
    moved_relationships = move_relationships(db, actor, primary, duplicate)
    ...
    _emit_event(db, actor, "ContactDuplicateMerged", "Party", primary.id, {
        ...
        "moved_notes": len(moved_notes),
        "moved_groups": len(moved_groups),
        "moved_relationships": len(moved_relationships),
    })
