...
export function createContactGroupsManagerApi(token: string) {
  ...
  return {
    ...
    archive: (groupId: string) => request<ManagedContactGroup>(`/api/contacts/groups/${encodeURIComponent(groupId)}`, { method: "DELETE" }),
    restore: (groupId: string) => request<ManagedContactGroup>(`/api/contacts/groups/${encodeURIComponent(groupId)}/restore`, { method: "POST" }),
