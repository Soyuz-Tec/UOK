...
function installDataToolsApiMock() {
  ...
  const fetchMock = vi.fn(async (input: RequestInfo | URL, options: RequestInit = {}) => {
    ...
    if (path.startsWith("/api/contacts/teams")) return jsonResponse([{ id: "team-1", name: "Operations", description: "", status: "active", member_count: 0, members: [] }]);
    ...
    if (path === "/api/contacts/custom-fields") return jsonResponse(method === "POST" ? { id: "field-3", ...bodyOf(options), status: "active" } : [
      { id: "field-1", field_key: "account_tier", label: "Account tier", field_type: "text", applies_to: "all", required: false, status: "active", options: [] },
      { id: "field-2", field_key: "market_segment", label: "Market segment", field_type: "choice", applies_to: "all", required: false, status: "active", options: ["SMB", "Enterprise"] },
    ]);
    if (path.endsWith("/custom-fields")) return jsonResponse([{ id: "value-1", field_definition_id: "field-1", field_key: "account_tier", label: "Account tier", field_type: "text", value: "preferred" }]);
