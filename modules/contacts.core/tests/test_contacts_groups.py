...
def test_business_email_domain_groups_are_created_idempotently(client: TestClient) -> None:
    ...
    solo_domain = f"solo-{suffix}.business"
    first = command(
        client,
        ops,
        "CreateContact",
        {
            "display_name": f"Domain Person One {suffix}",
            "email": f"one@{business_domain}",
            "company_name": f"Acme Shipping {suffix}",
        },
        f"uok-domain-person-one-{suffix}",
    )
    ...
    second = command(
        ...
        "CreateContact",
        {
            "display_name": f"Domain Person Two {suffix}",
            "email": f"two@{business_domain}",
            "company_name": f"Acme Shipping {suffix}",
        },
        f"uok-domain-person-two-{suffix}",
    )
    ...
    singleton = command(
        ...
        ops,
        "CreateContact",
        {"display_name": f"Solo Domain {suffix}", "email": f"solo@{solo_domain}"},
    ...
    grouped = command(
        ...
        {},
        f"uok-domain-grouping-{suffix}",
    )
    assert grouped.status_code == 200, grouped.text
    result = grouped.json()["result"]
    ...
    rerun = command(
        ...
        "GroupContactsByBusinessEmailDomain",
        {},
        f"uok-domain-grouping-rerun-{suffix}",
    )
