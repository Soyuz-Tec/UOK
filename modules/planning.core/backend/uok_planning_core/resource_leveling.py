...
from .models import PlanningAssignment, PlanningTask
from .schedule_math import CalendarSpec, at_utc, end_for_start, shift_working
from .task_constraints import is_auto_scheduled


def level_resource_allocations(tasks: list[PlanningTask], assignments: list[PlanningAssignment], calendar: CalendarSpec) -> set[str]:
    changed: set[str] = set()
    ...
    for task in sorted(tasks, key=lambda item: (item.start_at.date().isoformat(), item.sort_order, item.created_at.isoformat())):
        if task.task_type in {"summary", "milestone"} or not is_auto_scheduled(task) or task.id not in assigned:
            ...
            continue
        shifted = _level_task(task, assigned[task.id], tasks, assignments, fixed_task_ids, calendar)
        if shifted:
            changed.add(task.id)
        fixed_task_ids.add(task.id)
    return changed
...
def _level_task(task: PlanningTask, task_assignments: list[PlanningAssignment], tasks: list[PlanningTask], assignments: list[PlanningAssignment], fixed_task_ids: set[str], calendar: CalendarSpec) -> bool:
    start = task.start_at.date()
    for _ in range(260):
        if not _overloaded(task.id, start, task.duration_days, task_assignments, tasks, assignments, fixed_task_ids, calendar):
            return _move(task, start, calendar)
        start = shift_working(start, 1, calendar)
    return False


def _overloaded(task_id: str, start: date, duration: int, task_assignments: list[PlanningAssignment], tasks: list[PlanningTask], assignments: list[PlanningAssignment], fixed_task_ids: set[str], calendar: CalendarSpec) -> bool:
    ...
    for assignment in task_assignments:
        current = start
        while current <= end:
            if calendar.is_working_day(current) and usage[(assignment.resource_id, current)] + assignment.allocation_percent > 100:
                return True
