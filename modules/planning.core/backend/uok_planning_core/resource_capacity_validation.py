...
def validate_resource_capacity_result(
    ...
    expected = _expected_loads(tasks, resources, assignments, calendar)
    ...
    for key in sorted(set(expected) & set(actual)):
        allocation, task_ids = expected[key]
        point = actual[key]
        ...
        if point.capacity_percent != DEFAULT_CAPACITY_PERCENT:
            issues.append(_issue("resource_capacity_mismatch", "Resource capacity does not match the declared default.", point.resource_id))
...
def _expected_loads(
    ...
    assignments: list[PlanningAssignment],
    calendar: CalendarSpec,
) -> dict[tuple[str, date], tuple[int, tuple[str, ...]]]:
    ...
    for assignment in assignments:
        ...
        if task is None or assignment.resource_id not in resource_ids:
            continue
    ...
    return {key: (value, tuple(sorted(contributors[key]))) for key, value in allocation.items()}
