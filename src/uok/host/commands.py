...
def _log_validation_error(
    ...
    error_response = (
        exc.response_body()
        if isinstance(exc, (CommandDomainError, CommandPreconditionError))
        else {"error": str(exc)}
    )
    failed = CommandLog(
...
def _authorized_replay_or_none(
    db: Session,
    actor: Actor,
    command_type: str,
    payload: dict[str, Any],
    ...
    existing = db.scalar(select(CommandLog).where(
        CommandLog.organization_id == actor.organization_id,
        CommandLog.idempotency_key == key,
    ))
...
def execute_command(
    ...
    try:
        ...
        db.commit()
        return {"idempotent": False, "command_id": log.id, "status": log.status, "result": result}
...
__all__ = ["COMMAND_PERMISSIONS", "command_handlers", "execute_command"]
