...
- The built static assets must stay within the checked-in ceilings: cumulative JavaScript `1000 KiB` raw and `290 KiB` gzip; entry JavaScript `250 KiB` raw and `75 KiB` gzip; every deferred JavaScript chunk `240 KiB` raw and `65 KiB` gzip; CSS `220 KiB` raw and `30 KiB` gzip. At least 12 JavaScript assets must prove that the eleven compile-time module workspaces remain split. The cumulative gzip adjustment accounts for independent compression overhead while reducing the initial entry to less than one third of the former monolith; a ceiling change remains a reviewed decision, not an automatic response to a regression.
