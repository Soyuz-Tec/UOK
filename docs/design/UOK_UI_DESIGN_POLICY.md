...
- Shared module-neutral workflow primitives for repeated module behavior, including workflow headers, search fields, inline field messages, async `alertdialog` confirmations, loading/error/empty feedback, module render-error containment, inline text editing, reusable in-workspace pop-ups for record detail/edit workflows, and reusable data tables with accessible resizable columns. Modal workspace pop-ups inherit bounded pointer, touch, and keyboard movement from the shared overlay primitive; modules must not recreate dialog focus, async confirmation, drag, or error-boundary behavior locally. Current homes: `web/src/shared/*`.
