...
- governed Planning Flow Board with four persistent server-ordered status lanes, compact schedule/readiness cards, explicit keyboard-safe Open and Move controls, server-authorized transitions, explicit accepted/rejected mutation outcomes, authoritative reload, live result announcements, mixed-direction text isolation, and focus recovery; review-only mode keeps inspection available and disables every move, while a missing flow contract fails visibly instead of rendering an empty board
