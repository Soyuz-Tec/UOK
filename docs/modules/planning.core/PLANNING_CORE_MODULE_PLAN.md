...
- governed Planning Flow Board with four persistent server-ordered status lanes and compact direct-manipulation cards: selecting the card opens the Inspector, selecting the heading invokes the shared inline editor, and dragging moves the task only to server-authorized stages. The former visible Open/Move footer is removed; a compact per-card More menu exposes the same permitted moves for keyboard, screen-reader, and non-drag users. Guarded mutations still require explicit accepted outcomes and authoritative reload before live success announcements and focus recovery. Mixed-direction text remains isolated, review-only cards remain inspectable without edit/drag/menu mutation affordances, and a missing flow contract fails visibly instead of rendering an empty board.
