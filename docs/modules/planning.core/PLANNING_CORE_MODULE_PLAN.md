...
- editable task grid and modal Inspector path for create, update, delete, hierarchy, status, progress, and task type; task deletion uses a confirmed destructive action that discloses subtree and relationship removal before the governed mutation runs
...
- governed Planning Flow Board with four persistent server-ordered status lanes and compact direct-manipulation cards: selecting the card opens the Inspector, selecting the heading invokes the shared inline editor, and dragging moves the task only to server-authorized stages. The former visible Open/Move footer is removed; a compact per-card More menu exposes the same permitted moves for keyboard, screen-reader, and non-drag users. On hover-capable fine pointers the pencil and More marks use progressive disclosure (hidden at rest, softly revealed over the card, and fully visible on direct hover or keyboard focus); touch, non-hover, and forced-colors contexts keep them visible. Guarded mutations still require explicit accepted outcomes and authoritative reload before live success announcements and focus recovery. Mixed-direction text remains isolated, review-only cards remain inspectable without edit/drag/menu mutation affordances, and a missing flow contract fails visibly instead of rendering an empty board.
- keyboard-safe Planning List built on the shared resizable data-table contract with localized schedule facts and an explicit Open action instead of pointer-only row activation
- Gantt bars
...
- server-owned planned/in-progress/blocked/complete status vocabulary, display labels, ordered flow read model, and transition policy with structured invalid-value/transition rejection
- reason-required draft/active/on-hold/completed/archived Planning project lifecycle; the low-frequency Plan actions surface exposes recoverable `Delete project` as the governed transition to archived and `Restore project` as archived-to-active, both with explicit reason capture, strong concurrency, and audit evidence. A successful delete selects the next active actor-visible project or returns to Portfolio when none remains. Archived schedules, history, and exact pre-archive command replays remain readable while non-transition writes fail closed, and internal purge stays outside the public transition API and hides exact replay without recovered identifiers
...
- recoverable Delete/Restore for Planning resources is deliberately deferred. ADR-0009 through ADR-0012 make resources direct schedule, assignment, calendar, baseline, and scheduler inputs, while the current resource tables have no lifecycle status or retention boundary. Adding Delete before an ADR defines referential behavior, archived-schedule semantics, retention, and restore validation would make historical and calculated schedules ambiguous; resource removal therefore remains fail closed.
