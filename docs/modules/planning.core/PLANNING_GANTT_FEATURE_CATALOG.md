...
| Feature | Target behavior | Status |
|---|---|---:|
| Board/List/Calendar/Workload/People/Dashboard views | Alternate views use the same validated schedule. Board renders the four server-ordered flow lanes with direct cards: card selection opens the Inspector, heading selection uses shared inline editing, and drag/drop moves only to a published transition target. The former visible Open/Move footer is absent; compact More actions retain permitted keyboard/screen-reader/non-drag moves. Fine-pointer cards progressively disclose the pencil and More marks from hidden at rest to soft card-hover and full direct-hover/focus states, while touch, non-hover, and forced-colors contexts keep both controls visible. Every status/title write uses the guarded task API, explicit mutation outcome, authoritative reload, announcement, and focus recovery; stale/failed writes cannot be inferred as successful from matching remote state, and a missing flow contract fails visibly. Review-only cards remain inspectable with no edit, drag, or move menu. Board/List isolate mixed-direction business text, and List uses the shared resizable table with explicit keyboard-safe Open actions. Workload includes independently validated daily resource load lanes and overload counts. | `runtime_proven` |
