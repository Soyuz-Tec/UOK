...
| `PLA-UX-007` | Governed Planning flow and keyboard-safe list | Backend policy/OpenAPI tests publish the ordered literal transition contract; focused model/component tests cover direct card opening, allowed/invalid drag targets, compact non-drag move actions, inline heading validation/save/stale failure, explicit accepted/rejected outcomes, missing-contract alert, review-only behavior, localized/mixed-direction cards, list actions, announcements, and focus. Chromium proves guarded title/status PATCH headers, authoritative reloads, fine-pointer progressive action disclosure, touch-visible/tappable actions, card-to-Inspector interaction, review-only no-write behavior, Inspector focus return, and clean console/page state. | `runtime_proven` |
