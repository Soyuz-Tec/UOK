...
| CI gates | Python compile, Ruff, mypy, sequential branch coverage, dependency audits, technology audit, engineering evidence v2, database-security inventory, module contract, source-size hard caps and no-growth ratchet, source-boundary, naming, frontend lint/type/tests/coverage, PostgreSQL 18, disposable API candidate, hosted Chromium proof, exact-image Trivy scan, static build, and OpenSSF Scorecard |
