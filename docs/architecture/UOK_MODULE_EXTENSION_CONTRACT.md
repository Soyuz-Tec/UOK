...
- Module → Host production exceptions are path-and-symbol exact:
  `uok.host.database.get_db` and `uok.host.security.current_actor` in the 16
...
- `calendar.core`, `communications.core`, `contacts.core`, `locations.core`, `planning.core`, `product.master`, `reports.core`, and `routes.core` are optional `runtime_proven` capability modules, and `shipments.core` is an optional `runtime_proven` business module. All have manifest-declared backend hooks and module-owned verifiers.
...
- Apps Manager, Calendar, Communications, Contacts, Location Master, Planning, Product Master, Route/Corridor Master, and Shipment Support declare `web_surface`; their React source, module-local CSS, entrypoints, and frontend tests live below the owning module roots and are composed through the generated compile-time catalog.
