...
|---|---|---|
| `Party` / `parties` read in `modules/planning.core/backend/uok_planning_core/link_resolver.py` | `modules/contacts.core/backend/uok_contacts_core/public_api.py` owns the query and returns an immutable value DTO | `_resolve_party` imports only `uok_contacts_core.public_api` |
| `ReportArtifact` / `report_artifacts` read in `modules/planning.core/backend/uok_planning_core/link_resolver.py` | `modules/reports.core/backend/uok_reports_core/public_api.py` owns the query and returns an immutable value DTO | `_resolve_artifact` imports only `uok_reports_core.public_api` |
| `CalendarEvent` / `calendar_events` read in `modules/planning.core/backend/uok_planning_core/link_resolver.py` | `modules/calendar.core/backend/uok_calendar_core/public_api.py` owns the query and returns an immutable value DTO | `_resolve_calendar_event` imports only `uok_calendar_core.public_api` |
| `CommunicationThread` / `communication_threads` read in `modules/planning.core/backend/uok_planning_core/link_resolver.py` | `modules/communications.core/backend/uok_communications_core/public_api.py` owns the query and returns an immutable value DTO | `_resolve_communication_thread` imports only `uok_communications_core.public_api` |
| Broad Calendar facade import in `modules/planning.core/backend/uok_planning_core/calendar_bridge.py` | The same Calendar-owned occurrence/free-busy DTO queries are re-exported from `uok_calendar_core.public_api` | `calendar_bridge.py` imports only the exact owner public API surface |
