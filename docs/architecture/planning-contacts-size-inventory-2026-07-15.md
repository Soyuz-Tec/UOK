...
|---|---|---|:---:|---|
| Planning | `modules/planning.core/manifest.yaml:90` → `uok_planning_core.api:router` | Runtime composition through `pre-gap3:src/uok/module_routers.py`; Planning-owned HTTP clients consume the 44 routes. | Y | Expose only the composed router through `uok_planning_core.public_api`; route handlers, schemas, and subrouters become internal. |
...
| Planning | `uok_planning_core.commands::{command_handlers, command_permissions}` declared at `modules/planning.core/manifest.yaml:91-92` | Runtime command composition through `pre-gap3:src/uok/module_commands.py`; 30 declared commands. | Y | Keep only the two provider functions on `public_api`; all concrete handlers become internal. |
...
| Planning | `uok_planning_core.policy:role_grants` at `manifest.yaml:94` | Runtime permission composition through `pre-gap3:src/uok/module_policy.py`. | Y | Re-export through `public_api`; grant data stays internal. |
| Planning | `uok_planning_core.reports::{dashboard_counts, evidence}` at `manifest.yaml:95-96` | Runtime dashboard and evidence composition through `pre-gap3:src/uok/module_reports.py`. | Y | Re-export only these two provider functions through `public_api`. |
| Planning | `uok_planning_core.models:owned_models` at `manifest.yaml:97`, plus 18 ORM/helper names in that module's `__all__` | Privileged ORM registration through `pre-gap3:src/uok/module_model_registry.py`; compatibility-only assertions in `tests/test_module_model_registry.py`. | N | Manifest-resolved internal persistence/bootstrap SPI. It must not appear in the business `public_api` because it returns ORM mappings. |
...
| Contacts | Eight Contacts request classes formerly re-exported through a kernel Contact-schema shim, `src/uok/api/schemas.py`, and `pre-gap3:src/uok/main.py` | No repository production caller; the Contacts router consumes its owner schemas directly. | N | Keep request schemas in the Contacts internal delivery package and remove kernel re-exports. |
