...
- Planning, Contacts, Compliance, Product Master, Location Master, Route/Corridor Master, and Shipment Support expose supported Python facades at `uok_planning_core.public_api`, `uok_contacts_core.public_api`, `uok_compliance_core.public_api`, `uok_product_master.public_api`, `uok_locations_core.public_api`, `uok_routes_core.public_api`, and `uok_shipments_core.public_api`. All other implementation code is capability-organized below the owner's `_internal` package; external static imports are rejected by `tests/test_module_public_api_boundaries.py`.
- Module-owned ORM mappings remain private even when the model registry resolves their privileged manifest `model_exports` hook. Planning, Contacts, Compliance, Product Master, Location Master, Route/Corridor Master, and Shipment Support export no ORM mapping through a public facade.
...
- Static runtime validation completes before extension imports. The host-owned deterministic model registry then composes 53 module-owned mappings with nine kernel mappings (62 total) on the single `uok.kernel.persistence.Base` before migration inspection, schema creation, or router composition. Current closed manifests declare 105 commands, 115 events, 17 exact Host HTTP adapters with 37 exact allowed imports, 11 candidate verifiers, and 10 workbench surfaces.
- Apps Manager, Calendar, Communications, Compliance, Contacts, Planning, Product Master, Location Master, Route/Corridor Master, and Shipment Support own executable React source and local CSS under `modules/<module_name>/web/src`; their frontend tests live under `modules/<module_name>/tests/web`.
...
- Compliance Document Types, Product Master, Location Master, Route/Corridor Master, and Shipment Support own their complete DTO, HTTP, state, command, and workbench surfaces under their module web roots. Reports owns the typed report HTTP client under `modules/reports.core/web/src` without declaring a workbench surface. `agents.core` remains an inert planned scaffold with no executable frontend entry.
