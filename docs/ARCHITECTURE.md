...
- `shipments.core` is an optional business module for tenant-scoped operational Shipment headers, optimistic updates, and append-only movement-status history. It stores stable Party, Location, and optional Route IDs, resolves them only through immutable owner facades, and owns no Product/Cargo lines, booking, rates, tracking, documents, inventory, customs, or intelligence behavior.
...
- Static runtime validation completes before extension imports. The host-owned deterministic model registry then composes 51 module-owned mappings with nine kernel mappings on the single `uok.kernel.persistence.Base` before migration inspection, schema creation, or router composition.
