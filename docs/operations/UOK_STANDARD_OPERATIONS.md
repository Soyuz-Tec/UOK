...
| `Audit` | Full local CI-quality lane: Ruff, scoped mypy, sequential Python coverage, database/release policy, frontend dependency policy, ESLint, Stylelint, type/tests/coverage/accessibility, contracts, source size, naming, and folder organization | `.\scripts\uok_ops.ps1 -Action Audit` |
| `Verify` | Full audit plus static build, bundle budgets, UI proof, and isolated candidate verifier | `.\scripts\uok_ops.ps1 -Action Verify` |
